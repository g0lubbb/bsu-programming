...
/*
    ...
    try {
        ...
        std::vector<Point> points = getPoints(segments);
        write(theOStream, "Points - ends of the segments", points, ", ");
        ...
        std::set<Point> pointsInWindow = findPointsInWindow(codePoints);
        write(theOStream, "Points are in the visibility window", pointsInWindow, "\n");

        // 8. Находим список точек (концов отрезков), лежащих вне окна видимости
        std::set<Point> pointsOutsideWindow = findPointsOutsideWindow(codePoints);
        write(theOStream, "Points are outside the visibility window", pointsOutsideWindow, "\n");
        ...
        std::set<Segment> segmentsInWindow = findSegmentsInWindow(uniqueSegments, codePoints);
        write(theOStream, "Segments are in the window", segmentsInWindow, "\n");

        // 11. Находим коллекцию отрезков вне области видимости
        std::set<Segment> segmentsOutsideWindow = findSegmentsOutsideWindow(uniqueSegments, codePoints);
